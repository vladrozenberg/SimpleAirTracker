...
def ping_test_5_mins(plane_callsign):
    now=time.time()
    next_run = now -0.001
    test_time=0
    current_sec = now % 60
...
def get_plane_dictionary(plane_callsign):
    data = opensky_ping()
    plane_dictionary = {}
    for state in data['states']:
        if state[CALLSIGN].strip() == plane_callsign.strip():
            info={
    ...
    plane_dictionary[plane_callsign] = info
    print(plane_dictionary)
    return plane_dictionary


def main():
    plane_callsign = "DAL985"
    get_plane_dictionary(plane_callsign)
    #ping_test_5_mins(plane_callsign)
